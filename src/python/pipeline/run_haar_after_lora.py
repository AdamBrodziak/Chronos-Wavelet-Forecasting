"""
Pipeline: Haar-after-LoRA — Dekompozycja -> LoRA per poziom -> predykcja per poziom -> złożenie.
...
from common.config import HORIZONS, VARIANT_HAAR_AFTER_LORA, MODELS_DIR, DEFAULT_LORA_SPLIT_RATIO
...
def run(
    ...
    model_save_prefix: str = "chronos2_haar_after_lora",
    ...
    Osobny model LoRA jest trenowany na zrekonstruowanym w czasie sygnale z każdego wybranego poziomu Haara.
    Następnie każdy model predykuje swój poziom, a wyniki są sumowane na osi czasu.
    """
    if horizons is None:
        horizons = HORIZONS
    ...
    for level_name in selected_levels:
        ...
        if model_path.exists() and not force_retrain:
            ...
            train_ft, val_ft = split_train_test(level_train, test_ratio=DEFAULT_LORA_SPLIT_RATIO)
