"""
Pipeline: Haar-after — Dekompozycja -> oddzielne złożenie wybranych poziomów -> predykcja wybranych poziomów -> suma sygnałów.
...
from common.config import HORIZONS, VARIANT_HAAR_AFTER
...
def run(
    ...
    variant_name: str = VARIANT_HAAR_AFTER,
):
    """
    Pipeline Haar-after — predykcja po złożeniu wybranych poziomów dekompozycji Haara.

    1. Dekompozycja Haara na pełnych danych (train + test)
    2. Wybranie poziomów do dekompozycji Haar'a
    3. Rekonstruuje sygnał w dziedzinie czasu z tego jednego poziomu
    4. Uruchamia rolling window predykcję na tym sygnale
    5. Sumuje predykcje ze wszystkich poziomów -> finalna predykcja
    """
    if horizons is None:
        horizons = HORIZONS
    if selected_levels is None:
        # Domyślnie: wszystkie poziomy
        full_data = np.concatenate([y_train, y_test]) #TODO w przyszłości naszykować na wiele sygnałów
